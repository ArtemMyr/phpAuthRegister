!(function($) {
    ...
    script.ajaxform = {
        ...
        initHandler: function() {
            this.$forms.submit(function() {
                let $_this = $(this);
                let result = script.ajaxform.validate($_this);
        ...
        callback: function($form, data) {
            let action = getURLParam('act', $form.attr('action'));
            ...
            if (action !== ';' && action.length) {
                let callbackFunction = script.ajax.callbacks[action];
        ...
        go: function($form) {
            ...
            let $formButtons = $form.find(':button,input[type="submit"]');
    ...
    function tr(params, lang) {
            ...
            if (messages[lang] !== undefined && params.length) {
                for (let i = 0, msgcat = messages[lang]; i < params.length; i++) {
                    code = params[i];
